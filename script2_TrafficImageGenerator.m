...
for iPL=1:numel(platesIds.Files)
    numBG = randi([1 numel(backgroundIds.Files)]);
    bg_orig = readimage(backgroundIds, numBG);
    bg = imresize(bg_orig, [360 480], 'bilinear'); % Inputgroesse des SegNets
...
%         pause(1)
    ...
    filename = char(name + "_" + num2str(numBG) + ".png");
